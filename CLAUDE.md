...
MSBUILD=$("$VSWHERE" -latest -requiresAny \
  -requires Microsoft.Component.MSBuild \
  -find "MSBuild\**\Bin\amd64\MSBuild.exe" -nologo | head -1)
```
(Phase 12 문서 최종화 시점에 재검증: 옵션 이름이 `-requireAny`가 아니라 **`-requiresAny`**여야 하고, `-nologo` 없이는 vswhere 버전 배너 텍스트가 `$MSBUILD`에 그대로 섞여 들어가 이후 명령이 전부 깨진다 — 이전 버전 문서에 있던 오타를 여기서 수정함. 검증 시점 실제 경로는 `C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\amd64\MSBuild.exe` 였음)
...
```
(Phase 12 문서 최종화 시점에 재검증: 마지막 `0`이 하나 빠져 있으면 — 즉 하위 메뉴(`1`)를 빠져나오는 `0`까지만 주고 메인 메뉴 종료용 `0`을 안 주면 — 표준입력이 EOF에 도달한 뒤 메인 루프가 종료되지 않고 계속 빈 입력을 읽으며 도는 문제가 있었다. 하위 메뉴 진입 depth만큼 `0`을 하나씩 더 줘야 실제로 프로세스가 종료된다.)
