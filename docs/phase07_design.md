...
## 구현 결과 (완료)
- 설계와 달리 **`run()`이 곧바로 반환되는 대신, 결과 처리 후 "[Enter] 뒤로가기" 프롬프트로 명시적 대기**를 추가했다. 이유: `MainController::run()`이 루프 상단에서 항상 화면을 클리어하기 때문에, 결과를 보여준 즉시 반환하면 Phase 6에서 겪은 것과 같은 "메시지가 보이기도 전에 지워지는" 문제가 재발한다. 사용자가 Enter를 눌러야 실제로 메인 메뉴로 돌아가도록 해서 이를 방지했다.
- `orderStatusToString`/`orderStatusFromString`을 `model/OrderStatusText.h/.cpp`로 추출, `JsonOrderRepository`가 이를 사용하도록 리팩터링. `ConsoleOrderView`의 "현재 상태" 출력에도 재사용.
- `IOrderView`/`ConsoleOrderView`, `OrderController` 구현. `MainController`에 `order_menu`(세 번째 trailing default 파라미터) 연결.
- gmock 테스트 36개 전체 통과(`OrderControllerTest` 6개 신규: 정상 확정, 취소, 미등록 시료ID, 잘못된 수량 파싱 실패/0, run() 뒤로가기 대기).
- 실제 실행으로 등록된 시료 주문 → 확인 화면 → Y 확정 → 주문번호/상태 표시 → Enter 뒤로가기 → 메인 메뉴 반영(전체 주문 1건) 확인. N 취소 시 주문이 저장되지 않고 그대로 유지되는 것도 확인.
